...
<main class="min-h-screen bg-[#14161a] px-10 mt-28">
	{#if current}
		<button
			class="mb-8 px-6 py-2 cursor-pointer text-white rounded-lg font-semibold hover:bg-[#6a2fc9] transition"
			on:click={() => (window.location.href = `/properties/${current.name}`)}
		>
			&#8592; Back to {current.name}
		</button>
	{:else}
		<div class="mb-8 px-6 py-2 text-white rounded-lg font-semibold bg-red-600">
			Property not found. <a href="/properties/Ready%20to%20invest" class="underline"
				>Go to default property</a
			>
		</div>
	{/if}
	<div class="grid grid-cols-2 gap-10">
		...
		<div class="flex flex-col gap-4">
			<div class="relative">
				{#if images.length > 0}
					<img
						...
						class="w-full h-100 object-cover rounded-lg shadow-lg transition-transform duration-300 hover:scale-105 mb-2"
					/>
					<button
						class="absolute top-1/2 left-2 -translate-y-1/2 bg-[#222] p-2 rounded-full shadow hover:bg-[#883bed]"
						on:click={prevImage}
					...
					<button
						class="absolute top-1/2 right-2 -translate-y-1/2 bg-[#222] p-2 rounded-full shadow hover:bg-[#883bed]"
						on:click={nextImage}
					...
					<span
						class="absolute bottom-2 right-4 bg-[#883bed] text-white text-xs px-2 py-1 rounded-full"
					>
						{mainImageIndex + 1}/{images.length}
					</span>
				{:else}
					<div
						class="w-full h-56 flex items-center justify-center bg-gray-800 rounded-lg text-gray-400"
					>
						No images available
					</div>
				{/if}
			...
			<div class="flex gap-2">
				{#each images as img, i}
					<button
						type="button"
						class="cursor-pointer border-2 rounded-lg {mainImageIndex === i
							? 'border-[#883bed]'
							: 'border-transparent'}"
						on:click={() => (mainImageIndex = i)}
		...
		<div class="flex flex-col gap-4 justify-start">
			<span
				class="inline-block bg-[#8fff8f14] text-[#0ab10a] px-3 py-1 rounded-2xl w-fit font-semibold"
				>{details.rent || 'Rented'}</span
			>
			<p class="flex text-[.9rem] gap-1 items-center text-gray-400">
				<MapPin {size} />
				{details.location}
			</p>
			<p class="text-gray-400 mb-2">{details.place}</p>
			<div class=" rounded-lg p-4 shadow-lg">
				<p class="text-sm text-gray-400 mb-2 w-full flex justify-between">
					Funding Progress <span class="text-[#ffff] font-bold">{details.progress}</span>
				</p>
				<div class="bg-gray-700 w-full h-2 rounded relative mb-2">
					<div
						class="bg-[#883bed] h-2 rounded"
						style={`width: ${parseFloat(details.progress)}%`}
					></div>
				</div>
			</div>
			...
			<div class="bg-[#222] roundered p-6 shadow-lg mt-4">
					...
					<div class="flex items-center justify-between flex-col rounded-lg py-2">
						<span class="flex items-center text-[.8rem] gap-2 text-gray-300">
							...
							<svg
								...
								viewBox="0 0 24 24"
								stroke="currentColor"
								><circle cx="12" cy="12" r="10" stroke-width="2" /><text
									x="12"
									y="16"
									text-anchor="middle"
									font-size="12"
				...
				<button
					class="mt-4 w-full bg-[#883bed] text-white py-3 rounded-lg font-semibold hover:bg-[#6a2fc9] transition cursor-pointer"
					on:click={() => showInvestPopup = true}
				>
					Invest Now
				</button>
				<InvestPopup open={showInvestPopup} onClose={() => showInvestPopup = false} />
			</div>
		</div>
	</div>
